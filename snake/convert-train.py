...
def get_args():
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default='data/trajectories/snake/train',
...
def to_json(id, figures, meta_prompt, grid, output):
    return {
    "id": str(id),
    "image": f"{figures}",
    "conversations": [
    ...
    for trial in tqdm(trials):
        ...
        for file_idx in range(len(txt_files)-1):
            ...
            if file_idx > 0:
                ...
                t = trial.replace("_", "")
                conversation = to_json(f"{t}_step{file_idx}", img_path, meta_prompt, last_grid, response)
