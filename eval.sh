...
for IDX in $(seq 0 $((CHUNKS-1))); do
    ...
    --model-path checkpoints/$CKPT \
    --model-base liuhaotian/llava-v1.5-7b \
    --question-file ./data/eval/pope/$SPLIT.jsonl \
    --image-folder ./data/eval/pope/val2014 \
    --answers-file ./data/eval/pope/answers/$CKPT/${CHUNKS}_${IDX}.jsonl \
    --num-chunks $CHUNKS \
    --chunk-idx $IDX \
    --temperature 0 \
    --conv-mode vicuna_v1 &
...
# Loop through the indices and concatenate each file.
for IDX in $(seq 0 $((CHUNKS-1))); do
    cat ./data/eval/pope/answers/$CKPT/${CHUNKS}_${IDX}.jsonl >> "$output_file"
done
